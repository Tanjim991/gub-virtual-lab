document.addEventListener('DOMContentLoaded', () => {
    ...
    const userRole = sessionStorage.getItem('gub_user_role') || 'ADMIN';
    ...
    // ============================================================
    // ROLE-BASED ACCESS: TEACHER vs ADMIN (CEO vs Manager)
    // ============================================================
    if (userRole === 'TEACHER') {
        // Show TEACHER_NODE in sidebar
        const brandLabel = document.querySelector('.brand-mini p');
        if (brandLabel) brandLabel.textContent = 'TEACHER_NODE';

        // Hide BAN HAMMER — only CEO (Admin) can ban
        const banSection = document.getElementById('ban-section');
        if (banSection) banSection.style.display = 'none';

        // Hide Face Biometric Enrollment — only CEO has biometric
        const bioSection = document.getElementById('biometric-section');
        if (bioSection) bioSection.style.display = 'none';

        // Rename Super Admin panel header for teachers
        const superHeader = document.querySelector('#super-admin-panel h2');
        if (superHeader) {
            superHeader.textContent = 'TEACHER MANAGEMENT CENTER';
            superHeader.style.color = 'var(--gub-accent)';
        }
        const superDesc = document.querySelector('#super-admin-panel header p');
        if (superDesc) superDesc.textContent = 'Appoint qualified students as Teachers.';
        const superPanel = document.getElementById('super-admin-panel');
        if (superPanel) {
            superPanel.style.borderColor = 'var(--gub-accent)';
            superPanel.style.background = 'rgba(0,240,255,0.02)';
        }
        // Rename nav button too
        const superNavBtn = document.querySelector('.nav-btn[data-target="super-admin-panel"]');
        if (superNavBtn) {
            superNavBtn.textContent = ':: TEACHER_MGMT';
            superNavBtn.style.color = 'var(--gub-accent)';
        }
    }
